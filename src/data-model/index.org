...
If =A= is a set, then 
    ...
    3. =A+= denotes at least one element of =A=
    4. =A > B= denotes a relation /from/ =A= /to/ =B=.
    5. =r: A > B= denotes a relation from =A= to =B=.
...
*** Other constraints
There is a predefined user called =guest= always a member of
=user-set=.

#+begin_src spec :tangle spec.txt
 guest    : user-set    ; guest is a member of user set.
#+end_src
...
** Derived Entity sets
 ...
 role-set    : set[Role]    = user-set.roles.union        ; set of all roles of all users
...
[roles, users]   :  user-set+       > roles-set+   
[role, session] :  session-set     >  role-set! ;a session has one role
#+end_src


** Each user may belong to only one session unless the user is =guest=.
#+begin_src spec :tangle spec.txt
user: session-set > user-set!
session: {guest}  > session-set              ; a guest user could be in multiple sessions.
session: user-set \ {guest}  > session-set?  ; non-guest user is in at most one session.
#+end_src
...
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
| Object      | operation                                  | Credentials      | Ref. Integrity            | Result       |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
| user-set    | addUser(u:User, s:Session)                 | s.role=admin     | u:~user-set               | Succ or      |
|             |                                            |                  |                           | Failure      |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
...
|             |                                            |                  |                           | or Null      |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
| user        | makeUser(e: Email, n:Name, s: Session)     | s.role = admin   | e:~email-set              |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
|             | getEmail(u: User, s:Session)               | s.role >= guest  | u:user-set                |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
|             | getName(u: User,  s:Session)               | s.role >= guest  | u:user-set                |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
|             | getRoles(u: User, s:Session)               | s.role >= guest  | u:user-set                |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
|             | setEmail(u: User, e:Email, s:Session)      | s.role =admin OR | u:user-set                |              |
|             |                                            | s.user = u       |                           |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
|             | setName(u: User, n:Name, s:Session)        | s.role =admin OR | u:user-set                |              |
|             |                                            | s.user = u       |                           |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
|             | setRoles(u: User, rs:set[Role], s:Session) | s.role=admin     | u:user-set                |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
|             | addRole(u: User, r:Role, s:Session)        | s.role=admin     | u:user-set AND            |              |
|             |                                            |                  | r:~u.roles                |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
|             | delRole(u:User, r:Role, s:Session)         | s.role=admin     | u:user-set                |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
| session     | make-Session(u: user-set, r: Role)         |                  | r:u.roles                 |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
| session-set | addSession(ns:Session)                     |                  | ns.user = guest OR        |              |
|             |                                            |                  | ns.user:~session-set.user |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
|             | delSession(es:Session, s:Session)          | s.user=es.user   | es:session-set            |              |
|-------------+--------------------------------------------+------------------+---------------------------+--------------|
